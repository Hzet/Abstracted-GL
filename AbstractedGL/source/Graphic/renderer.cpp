...
namespace agl
{
	...
	{
		AGL_ASSERT(shaders_.find(name) == shaders_.end(), "Shader already added!");
	...
	{
		AGL_ASSERT(shaders_.find(name) != shaders_.end(), "Shader does not exist!");
	...
	{
		AGL_ASSERT(shaders_.find(active_) != shaders_.end(), "Shader does not exist!");
