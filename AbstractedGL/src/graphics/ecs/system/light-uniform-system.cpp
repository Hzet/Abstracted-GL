...
namespace agl
{
	...
	void light_uniform_system::update(registry &reg)
	{
		...
		static auto spot_view = reg.inclusive_view<uniform_type<spot_light>>();
		auto reindex_spot_view = true;
		auto index = 0;

		if (spot_view.needs_update())
		{
			spot_view = reg.inclusive_view<uniform_type<spot_light>>();
		...
		for (auto it = spot_view.begin(); it != spot_view.end(); ++it)
		{
			...
			auto& light = reg.get<uniform_type<spot_light>>(e);
