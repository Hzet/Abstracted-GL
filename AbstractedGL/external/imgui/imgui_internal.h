...
// enable_feature SSE intrinsics if available
...
// enable_feature stb_truetype by default unless FreeType is enabled.
...
//#define IMGUI_DEBUG_LOG_POPUP         IMGUI_DEBUG_LOG // enable_feature log
//#define IMGUI_DEBUG_LOG_NAV           IMGUI_DEBUG_LOG // enable_feature log
...
struct ImGuiContext
{
    ...
    ImGuiNavLayer           NavLayer;                           // Layer we are navigating on. For now the sys is hard-coded for 0=main contents and 1=menu/title bar, may expose layers later.
    ...
    // Legacy Focus/Tabbing sys (older than Nav, active even if Nav is disabled, misnamed. FIXME-NAV: This needs a redesign!)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    int                     FocusCounterRegular;    // (Legacy Focus/Tabbing sys) Sequential counter, start at -1 and increase as assigned via FocusableItemRegister() (FIXME-NAV: Needs redesign)
    int                     FocusCounterTabStop;    // (Legacy Focus/Tabbing sys) Same, but only count widgets which you can Tab through.
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_SaveSettings               = 1 << 22   // FIXME: Settings are handled by the docking sys, this only request the tab bar to mark settings dirty when reordering tabs
};
...
#define IMGUI_TEST_ENGINE_ITEM_ADD(_BB,_ID)                 if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemAdd(&g, _BB, _ID)               // register_uniform item bounding box
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID,_LABEL,_FLAGS)      if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS)   // register_uniform item label and status flags (optional)
