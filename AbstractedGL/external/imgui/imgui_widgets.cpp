...
// Then later the same sys could be used for multiple menu-bars, scrollbars, side-bars.
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    // We've been using the equivalent of ImGuiSelectableFlags_SetNavIdOnHover on all Selectable() since early Nav sys days (commit 43ee5d73),
...
// The *TabId fields be already set by the docking sys _before_ the actual TabItem was created, so we clear them regardless.
