...
template <typename TName>
std::string unique_type_id<TName>::get_name(std::uint64_t id)
{
#ifdef AGL_DEBUG

	if (id == 0)
		return "invalid";
	else if (id > unique_type_id_debug::get_max_id())
		return "unregistered";
...
std::uint64_t& unique_type_id<TName>::incrementor()
{
	...
	if (Result > unique_type_id_debug::get_max_id())
		unique_type_id_debug::get_max_id() = Result;
